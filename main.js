...
const starts = async (simple = new WAConnection()) => {
 ...
 simple.browserDescription = [ 'MDz', 'Browser', '3.0' ]
console.log(color('👋Allo Ges'))
console.log(color('TNGKYIU TOO'))
console.log(color(' >>>>>>¦¦¦¦¦¦¦¦|||||||||||||'))
console.log(color('│ 🐣 MoDz              '))
...
simple.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(simple.user.jid)) {
        simple.sendMessage(anu.jid, "Halo! Saya Bot WhatsApp untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(simple.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©MDz RxNc|| MDzTeam™", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(simple.user.jid)) {
        ...
        sendButImage(mdata.id, out, "© MoDzRxNc || MDzTeam™", buff, but)
      }
